...
public class ArticleServiceImpl extends ServiceImpl<ArticleMapper, Article> implements ArticleService {
    ...
    public IPage<UserEntity> allConnectionUser(Integer pageNum, Integer pageSize, Integer type) {
        ...
        if (type.equals(Constants.B_ONE)) {
            Set<String> fans = redisUtil.zrevrange(FANS + ShiroAuthenticationManager.getUserId(), 0, -1);
        ...
        if (type.equals(Constants.B_TWO)) {
            Set<String> followIds = redisUtil.zrevrange(FOLLOWING + ShiroAuthenticationManager.getUserId(), 0, -1);
            if (CollectionUtils.isNotEmpty(followIds)) {
                return userMapper.selectPage(page, new QueryWrapper<UserEntity>().in("user_id", followIds));
            }
        }
        return null;
    }
