...
//    }
    ...
    public void completeTask(Task task) {
        ...
        if(!StringUtils.equalsIgnoreCase(task.getParentId(),"0")){
            int one = count(new QueryWrapper<Task>().eq("parent_id", task.getParentId()).eq("task_status", false).eq("task_del",0));
            if (one == 0) {
                Task pTask = new Task();
    ...
    public void completionTaskInfo(Task task) {
        if (StringUtils.isNotEmpty(task.getTaskGroupId())) {
            //获取任务组名称
        ...
        task.setFileList(fileService.list(new QueryWrapper<File>().eq("public_id", task.getTaskId()).eq("public_lable", 1)
                .eq("file_del", 0)).stream().sorted(Comparator.comparing(File::getCreateTime)).collect(Collectors.toList()));
        //设置关联信息
